defmodule InstaWeb.HomeLive do
  use InstaWeb, :live_view

  alias Insta.Posts
  alias Insta.Posts.Post
  ...
  def render(assigns) do
    ...
    <div id="feed" phx-update="stream" class="flex flex-col gap-2"></div>
    ...
    <.button type ="submit" phx-disable-with="Saving ..."> Upload Post</.button>
    </.simple_form>
    </.modal>
    """
  end
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Insta.PubSub, "posts")
    ...
    |> case do
      {:ok, post} ->
        socket =
          ...
          Phoenix.PubSub.broadcast(Insta.PubSub, "posts", {:new, Map.put(post, :user, user)})
        {:noreply, socket}
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest = Path.join([:code.priv_dir(:insta), "static", "uploads", Path.basename(path)])
      File.cp!(path, dest)
